...
export async function generateMetadata({ params, searchParams }) {
  try {
    ...
    const { siteMeta } = await fetchPageData(subdomain, "book");

    return {
      title: `${siteMeta.title} - Book Appointment`,
...
export default async function BookPage() {
  try {
    ...
    const host = headersList.get("host") || "localhost:3000";
    const subdomain = getSubdomain(host);
    const { site, siteMeta, config, theme, page, sections } = await fetchPageData(subdomain, "book");
